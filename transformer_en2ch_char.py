...
# first
TRAIN_FILE = 'data/first/shanghai/data_en2ch/train_en2ch.txt'  # 训练集
DEV_FILE = "data/first/shanghai/data_en2ch/dev_en2ch.txt"      # 验证集
TEST_FILE = "data/first/shanghai/data_en2ch/test_en2ch.txt"
SAVE_FILE = 'data/first/shanghai/data_en2ch/model_en2ch_word.pt'         # 模型保存路径

# second。1
# TRAIN_FILE = 'data/second/order/ChemBook_no_split_dev/data_en2ch/train_en2ch.txt'  # 训练集
# DEV_FILE = "data/second/order/ChemBook_no_split_dev/data_en2ch/dev_en2ch.txt"      # 验证集
# TEST_FILE = "data/second/order/ChemBook_no_split_dev/data_en2ch/test_en2ch.txt"
# SAVE_FILE = 'data/second/order/ChemBook_no_split_dev/data_en2ch/model_en2ch_char.pt'         # 模型保存路径

# # second。2
# TRAIN_FILE = 'data/second/order/shanghai/data_en2ch/train_en2ch.txt'  # 训练集
# DEV_FILE = "data/second/order/shanghai/data_en2ch/dev_en2ch.txt"      # 验证集
# TEST_FILE = "data/second/order/shanghai/data_en2ch/test_en2ch.txt"
# SAVE_FILE = 'data/second/order/ChemBook_no_split_dev/data_en2ch/model_en2ch_char.pt'         # 模型保存路径
...
class PrepareData:
    ...
    #         "data/second/order/ChemBook_no_split_dev/data_en2ch/train_en2ch.txt")
    #     self.en_word_dict, self.en_total_words, self.en_index_dict = \
    #         self.build_dict(self.train_en)
    #     self.cn_word_dict, self.cn_total_words, self.cn_index_dict = \
    #         self.build_dict(self.train_cn)
    ...
    def load_data(self, path):
        ...
        with open(path, mode="r", encoding="utf-8") as f:
            for line in f.readlines():
                sent_en, sent_cn = line.split("\t")
                sent_en = sent_en.lower()
                sent_en = ["BOS"] + list(sent_en) + ["EOS"]
...
def evaluate(data, model):
    wb = op.load_workbook('data/second/order/ChemBook_no_split_dev/data_en2ch/en2ch_Result_char.xlsx')
    ws = wb.create_sheet('test_en2ch')
    ws.append(['Source', 'Targe', 'shanghai_Result'])
    with torch.no_grad():
        with open('data/second/order/ChemBook_no_split_dev/data_en2ch/predict_en2ch_char.txt', 'w', encoding="utf-8") as file:
            for i in range(len(data.test_en)):
                en_sent = " ".join([data.en_index_dict[w] for w in data.test_en[i]])
                en_sent2excle = en_sent[4:-3]
                print("英文：",en_sent2excle)
                cn_sent = " ".join([data.cn_index_dict[w] for w in data.test_cn[i]])
                cn_sent2excle = cn_sent[4:-3].replace(" ","")
                print("中文：",cn_sent2excle)
                ...
                print(translation)
                d = en_sent2excle, cn_sent2excle ,translation
                ws.append(d)
                wb.save('data/second/order/ChemBook_no_split_dev/data_en2ch/en2ch_Result_char.xlsx')
